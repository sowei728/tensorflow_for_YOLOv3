...
class yolov3(object):
    ...
    def compute_loss(self, pred_feature_map, y_true, ignore_thresh=0.5, max_box_per_image=8):
        ...
        print("*"*40)
        print("into compute_loss:")
        print("pred_feature_map0:",pred_feature_map[0].shape)
        print("pred_feature_map1:",pred_feature_map[1].shape)
        print("pred_feature_map2:",pred_feature_map[2].shape)

        print("y_true0:",y_true[0])
        print("y_true1:",y_true[1])
        print("y_true2:",y_true[2])
        ...
        # return [total_loss, loss_xy, loss_wh, loss_conf]



    def loss_layer(self, feature_map_i, y_true, anchors):
        print("into loss layer")
        print("feature map:", feature_map_i.shape)
        print("y_true:", y_true)
        ...
        print("reshaped y_true:", y_true.shape)
        print("object_mask:", object_mask.shape)
        print("valid_true_boxes:", valid_true_boxes.shape)
        ...
        # 在计算best_iou之后，由遮挡所带来的误差可能影响NMS对目标框的筛选，所以，本问题提出了对遮挡框的补偿机制，
        # 即基于特殊标记，计算互遮挡系数和背景遮挡系数
        ...
        # shape: [N, 13, 13, 3, 1]
        # class_loss =  （1 + a ） * object_mask * tf.nn.sigmoid_cross_entropy_with_logits(labels=y_true[..., 5:], logits=pred_prob_logits)
