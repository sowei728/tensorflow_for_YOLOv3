...
CLASS = [ "obscured(1)_apple", "obscured(2)_apple", "obscured(3)_apple","apple"]
voc_path = 'apple_dataset'
...
def get_boxes_for_id(voc_path,id):
    ...
    for obj in root.iter('object'):
        ...
        if label not in CLASS:  # exclude difficult or unlisted classes
            continue
...
def get_ids(voc_path,datasets):
    ...
    id_file = voc_path + '/ImageSets/{}.txt'.format(datasets)
    with open(id_file, 'r') as image_ids:
        line = image_ids.readline().split('.')[0]
        while line:
            ids.append(line)
            line = image_ids.readline().split('.')[0]
    return ids
